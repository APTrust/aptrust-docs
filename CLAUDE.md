...
- **`.github/workflows/build-and-deploy.yml`** builds and deploys to `gh-pages` via `peaceiris/actions-gh-pages` (force-orphan, custom domain `docs.aptrust.org`). It fires on: push to `main` in this repo, manual `workflow_dispatch`, `repository_dispatch` (`sub-repo-updated`) sent by each sub-repo's own `notify-parent-docs.yml` workflow when *its* main/master changes, and a nightly `schedule` at 07:00 UTC. That dispatch requires a fine-grained PAT (owned by the `aptdeploy` service account; Contents: Read and write on `APTrust/aptrust-docs`; 366-day expiry) stored as a secret in each sub-repo — but **the secret name differs per repo**: `DOCS_DISPATCH_TOKEN` in `userguide`, `registry-docs` and `preserv-docs`, `DOCS_DISPATCH_PAT` in `dart-docs`. Read the sub-repo's own `notify-parent-docs.yml` before touching either; a rotation that updates only `DOCS_DISPATCH_TOKEN` strands dart-docs on the dead token, and that fails silently — the sub-repo's notify run goes red, but the nightly build below re-clones all four sub-repos anyway, so the content still ships within 24 hours and nobody reports an outage. The nightly schedule exists because the Member API spec lives in `APTrust/registry`, which sends no dispatch here — on `schedule` and `workflow_dispatch` only, Step 5 commits the curled spec back to `main` when it differs, so the offline fallback copy stays current and spec changes get a git history. The commit lives in this workflow rather than a standalone sync workflow because `GITHUB_TOKEN` pushes don't trigger workflow runs (which is also why it can't loop) — a separate workflow would need a PAT to make the site redeploy.
...
- **The http→https redirect lives in Cloudflare, not in this repo — and GitHub's "Enforce HTTPS" can never be turned on here.** `gh api repos/APTrust/aptrust-docs/pages` reports `"https_enforced": false`, and `PUT`ting `https_enforced=true` fails with `The certificate does not exist yet` (404). The hostname is **proxied through Cloudflare** — `dig docs.aptrust.org` returns Cloudflare IPs (172.67.x / 104.21.x), TLS is terminated with a Google Trust Services cert for `CN=aptrust.org`, and the origin response still carries `x-github-request-id`. GitHub can't complete its own ACME challenge behind that proxy, so it will never provision the cert `https_enforced` requires; don't waste time retrying that API call. (The four sub-repos are *not* proxied and do have `https_enforced: true`, which is why they differ.) Until 2026-08-24 this meant `http://docs.aptrust.org/` served **200**, giving all ~169 pages a crawlable `http://` twin that canonicalised to the https URL. Resolved with a Cloudflare **Single Redirect** rule, scoped to this one hostname rather than the zone-wide *Always Use HTTPS* toggle, so the rest of the aptrust.org zone is untouched:
  - match (wildcard): `http://docs.aptrust.org/*` → target `https://docs.aptrust.org/${1}`, **301**, *Preserve query string* **on** (it defaults to off, which silently drops `?h=` search-highlight params)
  - verified: every path maps 1:1, one hop, `?h=` survives, https requests don't re-fire the rule, and `aptrust.org`/`www.aptrust.org` behave exactly as before

  If `http://docs.aptrust.org/` ever starts returning 200 again, look in the Cloudflare dashboard (Rules → Redirect Rules) — there is nothing in this repo that could cause or fix it.
- **URL prefixes** for each tab come from that sub-repo's `site_name` in its own `mkdocs.yml`, not from this repo's nav labels.
- **`mkdocs-monorepo-plugin` never runs a sub-repo's `plugins:` block** — it reads only `nav` and `docs_dir` out of each sub-repo's `mkdocs.yml`. `repos/userguide/mkdocs.yml` declares two `redirects` entries (`bagging/index.md` → `depositing/index.md`, `bagging/tools.md` → `depositing/tools.md`); those were silently dropped, so `/user-guide/bagging/` 404'd on the live site while `aptrust.github.io/userguide/bagging/` still forwarded traffic to it. They are re-declared in **this** repo's `mkdocs.yml`, rewritten against the *merged* tree — hence the `user-guide/` prefix on both sides of each mapping. `redirects` must stay **after** `monorepo` in the `plugins:` list: it resolves targets in `on_files` from `files.documentation_pages()` keyed by `src_path`, so the merge has to have happened first, and an unresolvable target is only a `Redirect target … does not exist!` warning, not a build failure. Any future sub-repo `plugins:` entry has the same problem and has to be lifted here the same way.
- **`search.share` is deliberately off** (removed 2026-08-24). Material's template ships the share control as `<a href="javascript:void(0)" data-md-component="search-share" data-clipboard-text="">`, but `bundle.js` **swaps those two attributes at runtime**: the `href` becomes `<current page>/?q=` and `data-clipboard-text` becomes `javascript:void(0)`. That left one real, crawlable self-link on every one of the ~169 pages. Googlebot renders JS, followed it, and Search Console began filing the `?q` URLs under "Alternate page with proper canonical tag" — harmless in itself (the canonical was honoured) but scaling to the whole site. Removing the feature from `theme.features` empties the `{% if "search.share" in features %}` guard in `overrides/partials/search.html`, so the anchor never reaches the DOM. Don't re-enable it without a plan for that link. `search.highlight` is unaffected: its `?h=` URLs only exist on client-rendered search results, which a crawler never triggers.
...
- **New sub-repo tab**: add an `!include` entry to `nav:` in `mkdocs.yml`; add a matching `git clone` line to the workflow's Clone step; add `notify-parent-docs.yml` + `DOCS_DISPATCH_TOKEN` to the new sub-repo (that name, not dart-docs's `DOCS_DISPATCH_PAT`); add a badge selector to `docs/stylesheets/extra.css`.
